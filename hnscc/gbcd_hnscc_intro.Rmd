...
LN metastasis), and shaped by tumor molecular subtype.
...
```{r summary-L, eval=FALSE}
head(round(fit.gbcd$L,digits = 3))
```

Create a heatmap to visualize GEP membership estimates.

```{r plot-L, fig.height=6, fig.width=11, eval=FALSE}
...
This heatmap shows the membership values of the $n = 2,176$ cells
(rows) for 28 GEPs (columns). Cells are arranged top-to-bottom by
tumor molecular subtype and patient of origin. Membership values are
rescaled separately for GEPs so that the maximum membership for each
GEP is always 1.
...
patient and can thus be interpreted as patient-specific GEPs,
including GEPs 4,7,9,11,25,27; the remaining GEPs are shared across
patients but do not correspond to molecular subtypes.
...
```{r plot-lfc, fig.height=7.5, fig.width=11, eval=FALSE}
