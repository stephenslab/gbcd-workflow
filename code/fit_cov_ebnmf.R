...
fit.cov.ebnmf <- function(Y, Kmax, prior = ebnm::ebnm_generalized_binary, thres = 0.8, extrapolate = TRUE, maxiter = 500, verbose = 1){
  ...
  fit.cov <- flash.init(dat, var.type = 0) %>% flash.init.factors(init = lapply(cov.init, function(x) x[, kmax, drop = FALSE]), 
                        ebnm.fn = ebnm::ebnm_unimodal_nonnegative) %>% flash.init.factors(init = lapply(cov.init, function(x) x[, -c(kmax), 
                        drop = FALSE]), ebnm.fn = prior) %>% flash.backfit(extrapolate = extrapolate, maxiter = 25, verbose = verbose) 
  ...
  fit.Ltilde <- fit.cov$F.pm[, k.order]
  ...
  fit.snmf <- flash.init(Y, S = 1/sqrt(nrow(Y)), var.type = 2) %>% flash.init.factors(
    init = list(as.matrix(fit.L), as.matrix(init.F)), ebnm.fn = c(prior, ebnm::ebnm_point_laplace)
    ) %>% flash.fix.factors(kset = 1:ncol(fit.L), mode = 1L) %>% flash.backfit(extrapolate = FALSE, verbose = verbose)
  
  ### calculate the z-score and lfsr of GEP signatures by running linear regression followed by ash
  J <- ncol(Y)
  genes <- colnames(Y)
  F.est <- matrix(0, J, ncol(fit.snmf$L.pm))
  rownames(F.est) <- genes
  colnames(F.est) <- colnames(fit.snmf$L.pm)
  F.se <- F.est
  F.z.ash <- F.est
  F.lfsr.ash <- F.est
  ...
  for (j in 1:J) {
    y     <- Y[,j]
    dat   <- as.data.frame(cbind(y, fit.snmf$L.pm))
    fit   <- lm(y ~ 0 + ., dat)
  ...
  L.pm <- fit.snmf$L.pm[, k.idx]
  colnames(L.pm) <- c("Baseline", paste0("GEP", 1:(ncol(L.pm)-1)))
  F.lfc <- fit.snmf$F.pm[, k.idx]/log(2)
  F.z <- F.z.ash[, k.idx]
...
init.cov.ebnmf <- function(fl, kset = 1:ncol(fl$flash.fit$EF[[1]])) {
...
fit.ebcovmf <- function(dat, fl, prior, extrapolate = TRUE, maxiter = 500, epsilon = 0, verbose = 1){
  ...
  while(s2 > 0 && abs(s2_diff - 1) > 1e-3) {
    ...
    fl <- flash.init(dat_minuss2) %>% flash.init.factors(init = lapply(fl$flash.fit$EF, function(x) x[, kmax, drop = FALSE]), 
                     ...
                     init = lapply(fl$flash.fit$EF, function(x) x[, kset, drop = FALSE]),
                     ebnm.fn = prior) %>% flash.backfit(extrapolate = extrapolate, maxiter = maxiter, verbose = verbose)
    old_s2 <- s2
    s2 <- max(0, mean(diag(dat) - diag(fitted(fl))))
    s2_diff <- s2 / old_s2
  }
